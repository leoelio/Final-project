...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate the ACT candidate trained on contact-stage scripted demos.")
    ...
    parser.add_argument("--output-csv", type=Path, default=ROOT / "docs" / "contact_stage_demo_torch_act_report.csv")
    parser.add_argument("--output-md", type=Path, default=ROOT / "docs" / "contact_stage_demo_torch_act_report.md")
...
def write_md(path: Path, args: argparse.Namespace, model_path: Path, model: dict, rows: list[dict], summary_rows: list[dict]) -> None:
    ...
    eval_command = ps_command("scripts/evaluate_contact_stage_demo_torch_act.py", ["--model", model_path])
    ...
    lines = [
        "# Contact-stage Demo Torch ACT 候选诊断报告",
        "",
        f"版本：`{VERSION}`",
        "",
        "定位：该版本使用 `contact_stage_demo_v1` 采集的 12 条 scripted contact-stage 成功示范训练 Torch ACT-style 动作块模型，用于检验“更干净示范和更平衡阶段分布”是否能改善 learned ACT 的闭环抓取。它仍是本地 state-only ACT-style 候选，不是完整官方 ACT，也不是 VLA 后训练。",
        "",
        "## 1. 数据与模型",
        "",
    ...
    lines.extend(
        [
            ...
            "## 5. 论文边界",
            "",
            "该版本只能写成“更干净 scripted 示范训练 ACT 仍需闭环验证”的候选诊断。若批量评测仍没有标准抓取或严格抓取成功，不能写成 ACT 已经学会 contact-stage 控制；若只出现目标距离成功，也必须和 `grasp_success/object_z` 分开解释。",
            "",
        ]
    )
