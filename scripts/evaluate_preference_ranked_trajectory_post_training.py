...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate ranked-preference trajectory post-training candidate.")
    ...
    parser.add_argument("--output-csv", type=Path, default=ROOT / "docs" / "preference_ranked_trajectory_post_training_report.csv")
    parser.add_argument("--output-md", type=Path, default=ROOT / "docs" / "preference_ranked_trajectory_post_training_report.md")
    return parser.parse_args()


def latest_model() -> Path:
    candidates = sorted(
        (ROOT / "outputs" / "preference_post_training").glob("preference_ranked_trajectory_post_training_*.npz"),
        key=lambda path: path.stat().st_mtime,
    )
    if not candidates:
        raise FileNotFoundError("no preference_ranked_trajectory_post_training model found")
    return candidates[-1]
...
def write_md(path: Path, args: argparse.Namespace, model_path: Path, model: dict, rows: list[dict], summary_rows: list[dict]) -> None:
    ...
    eval_command = ps_command("scripts/evaluate_preference_ranked_trajectory_post_training.py", ["--model", model_path])
    ...
    lines = [
        ...
        f"版本：`{VERSION}`",
        "",
        "定位：该版本把每个 episode 内的多个 attempt 按“最终是否成功、是否出界、目标距离”排序，把排序转成 trajectory-level preference weight，再用于相对几何增强的 trajectory-kNN 邻居投票。它是本地 MuJoCo 后训练候选，不是在线 RL、真实人类偏好优化或真实 VLA 后训练。",
        "",
        "## 1. 偏好来源与模型",
        "",
        f"- 模型：`{model_path.relative_to(ROOT).as_posix()}`",
        f"- preference_mode：`{metadata.get('preference_mode')}`",
    ...
    lines.extend(
        [
            ...
            "## 5. 论文边界",
            "",
            "可以写：episode 内排序偏好 + 相对几何 trajectory-kNN 的本地后训练候选已经完成闭环评测和视频诊断。",
            "",
            "不能写：在线 RL、真实人类偏好优化、OpenVLA LoRA/Adapter、真实 robot VLA 后训练或真实 WidowX 成功。若放置距离成功但 `grasp_success=False`，仍必须按严格抓取审计写成失败或负例。",
        ]
    )
    path.parent.mkdir(parents=True, exist_ok=True)
