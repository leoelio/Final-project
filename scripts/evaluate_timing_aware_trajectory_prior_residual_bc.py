...
import run_timing_aware_trajectory_prior_residual_policy as runner  # noqa: E402
from timing_aware_trajectory_prior_residual_common import VERSION, load_residual_model  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate timing-aware trajectory-prior residual BC and write a Chinese report.")
    ...
    parser.add_argument("--output-csv", type=Path, default=ROOT / "docs" / "timing_aware_trajectory_prior_residual_bc_report.csv")
    parser.add_argument("--output-md", type=Path, default=ROOT / "docs" / "timing_aware_trajectory_prior_residual_bc_report.md")
...
def write_md(path: Path, args: argparse.Namespace, model_path: Path, rows: list[dict], summary_rows: list[dict]) -> None:
    ...
    lines = [
        ...
        f"版本：`{VERSION}`",
        "",
        "定位：该版本属于 trajectory-conditioned BC / ACT 路线的候选诊断。它保留分阶段 scripted 轨迹先验 + residual BC action head 的结构，但把上一轮夹爪时序诊断中有效的强闭合、闭合保持和抬升保持写进轨迹阶段。",
        "",
        "边界：它不是完整官方 ACT，不是纯神经网络策略，不是 VLA / OpenVLA 后训练，也不能直接写成真实 WidowX 成功。它的作用是检验“接触时序先验能否改善 trajectory-conditioned learned residual 的闭环表现”。",
        "",
        f"模型：`{model_path}`",
        "",
        "## 1. 汇总结论",
    ...
    lines.extend(
        [
            ...
            "## 3. 可写结论和不可写结论",
            "",
            "- 可以写：该候选把接触时序先验并入 trajectory-conditioned residual BC，用于检验闭合保持是否改善闭环放置和 TCP 抬升。",
            "- 可以写：如果 held-out 仍下降，说明方法主要依赖几何/时序先验，泛化仍不足。",
            "- 不能写：完整 ACT、OpenVLA、LoRA/Adapter 或真实机器人后训练已经完成。",
            "- 不能写：只要 TCP 抬升为真就等于严格抓取成功，仍需同时报告 `grasp_success` 和 `strict_grasp_lift_success`。",
            "",
            "## 4. 完整命令",
            "",
            "训练命令：",
...
def main() -> None:
    ...
    print(f"timing_aware_trajectory_prior_residual_json: {args.output_json}", flush=True)
    print(f"timing_aware_trajectory_prior_residual_csv: {args.output_csv}", flush=True)
    print(f"timing_aware_trajectory_prior_residual_md: {args.output_md}", flush=True)
    print(f"timing_aware_trajectory_prior_residual_rows: {len(rows)}", flush=True)
