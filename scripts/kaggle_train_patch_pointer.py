...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-version", default="clip_patch_pointer_kaggle_v2_cpu_v1")
...
def encode(clip_model, processor, root: Path, examples: list[dict], batch_size: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    unique_images = {row["id"]: row for row in examples}
    patch_by_id = {}
    image_rows = list(unique_images.values())
    for start in range(0, len(image_rows), batch_size):
        ...
        with torch.no_grad():
            patches = clip_model.vision_model(pixel_values=inputs["pixel_values"]).last_hidden_state[:, 1:, :].cpu().numpy().astype(np.float32)
        patch_by_id.update({row["id"]: patch for row, patch in zip(batch, patches)})
    text_batches = []
    ...
    return (
        np.stack([patch_by_id[row["id"]] for row in examples]).astype(np.float32),
        np.concatenate(text_batches),
        np.asarray([int(row["task_label"]) for row in examples], dtype=np.int64),
        np.asarray([row["source_pixel_uv_normalized"] for row in examples], dtype=np.float32),
    )
...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch in {1, args.epochs} or epoch % 50 == 0:
            ...
            if validation_result["pointer_rmse_m"] < best_validation_rmse:
                best_epoch = epoch
                best_validation_rmse = validation_result["pointer_rmse_m"]
    ...
    metadata = {
        ...
        "best_validation_pointer_rmse_m": best_validation_rmse,
