...
VARIANTS: dict[str, TimingVariant] = {
    "baseline": TimingVariant(
        descend_gripper=0.037,
        close_gripper=0.015,
        close_steps=260,
        hold_after_close_steps=0,
        lift_hold_steps=0,
    ),
    "tight_close_hold": TimingVariant(
        descend_gripper=0.037,
        close_gripper=0.004,
        close_steps=420,
        hold_after_close_steps=280,
        lift_hold_steps=220,
    ),
    "early_close_hold": TimingVariant(
        descend_gripper=0.015,
        close_gripper=0.004,
        close_steps=220,
        hold_after_close_steps=260,
        lift_hold_steps=220,
    ),
}
...
class TimingTrace:
    ...
    min_ee_object_distance: float | None = None
    min_ee_object_distance_while_lifted: float | None = None
    first_lift_step: int | None = None
    first_tcp_lift_step: int | None = None
    first_standard_grasp_step: int | None = None
    ever_standard_grasp_success: bool = False
    ...
    stage_min_ee_distance: dict[str, float] = field(default_factory=dict)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a scripted gripper timing/contact diagnostic probe.")
    ...
    parser.add_argument("--variant", choices=sorted(VARIANTS), default="tight_close_hold")
...
def distances(env: WidowXTabletopEnv) -> tuple[float, float]:
    object_position = env.object_position(env.episode_target_object)
    tcp_distance = float(np.linalg.norm(env.tcp_position() - object_position))
    ee_distance = float(np.linalg.norm(env.data.xpos[env.ee_body_id] - object_position))
    return tcp_distance, ee_distance


def update_trace(args: argparse.Namespace, trace: TimingTrace, env: WidowXTabletopEnv, stage: str) -> None:
    metrics = env.metrics()
    object_z = float(metrics["object_z"])
    contact_count = float(metrics["contact_count"])
    tcp_distance, ee_distance = distances(env)

    trace.steps_taken += 1
    trace.max_object_z = max(trace.max_object_z, object_z)
    trace.max_contact_count = max(trace.max_contact_count, contact_count)
    trace.min_tcp_object_distance = tcp_distance if trace.min_tcp_object_distance is None else min(trace.min_tcp_object_distance, tcp_distance)
    trace.min_ee_object_distance = ee_distance if trace.min_ee_object_distance is None else min(trace.min_ee_object_distance, ee_distance)
    trace.stage_steps[stage] = int(trace.stage_steps.get(stage, 0)) + 1
    trace.stage_min_tcp_distance[stage] = min(float(trace.stage_min_tcp_distance.get(stage, float("inf"))), tcp_distance)
    trace.stage_min_ee_distance[stage] = min(float(trace.stage_min_ee_distance.get(stage, float("inf"))), ee_distance)
    ...
    if bool(metrics["grasp_success"]):
        trace.ever_standard_grasp_success = True
        if trace.first_standard_grasp_step is None:
            trace.first_standard_grasp_step = trace.steps_taken
    ...
    if lifted:
        ...
        trace.min_tcp_object_distance_while_lifted = (
            ...
            else min(trace.min_tcp_object_distance_while_lifted, tcp_distance)
        )
        trace.min_ee_object_distance_while_lifted = (
            ee_distance
            if trace.min_ee_object_distance_while_lifted is None
            else min(trace.min_ee_object_distance_while_lifted, ee_distance)
        )
        if tcp_distance < float(args.tcp_lift_threshold):
    ...
    for index in range(int(steps)):
        ...
        if args.log_every > 0 and trace.steps_taken % int(args.log_every) == 0:
            ...
            print(
                ...
                f"tcp_dist={tcp_distance:.4f} ee_dist={ee_distance:.4f} contact_count={float(metrics['contact_count']):.0f}",
                flush=True,
            )
...
def with_gripper(action: np.ndarray, value: float) -> np.ndarray:
    modified = action.copy()
    modified[6] = float(value)
    return modified


def execute_attempt(args: argparse.Namespace, env: WidowXTabletopEnv, expert: PickPlaceExpert, trace: TimingTrace, viewer=None) -> dict:
    ...
    plan = expert.plan(env.episode_target_object, env.task.target_geom)
    actions = plan["actions"]
    cfg = expert.config

    close_action = with_gripper(actions["grasp_closed"], variant.close_gripper)
    lift_action = with_gripper(actions["lift_closed"], variant.close_gripper)
    transfer_action = with_gripper(actions["transfer_closed"], variant.close_gripper)
    place_action = with_gripper(actions["place_closed"], variant.close_gripper)

    pre_lift_steps = [
        ("approach_open", actions["approach"], cfg.approach_steps),
        ("descend_timing", with_gripper(actions["grasp_open"], variant.descend_gripper), cfg.descend_steps),
        ("close_timing", close_action, variant.close_steps),
        ("hold_after_close", close_action, variant.hold_after_close_steps),
        ("lift_closed", lift_action, cfg.lift_steps),
        ("lift_hold", lift_action, variant.lift_hold_steps),
    ]
    ...
    metrics = env.metrics()
    tcp_distance, _ = distances(env)
    lifted_for_transfer = bool(float(metrics["object_z"]) >= float(args.lift_threshold) and tcp_distance < float(args.tcp_lift_threshold))
    if not lifted_for_transfer:
        return {"continued_to_place": False, "success": False, "lifted_for_transfer": False}

    place_steps = [
        ("transfer_closed", transfer_action, cfg.transfer_steps),
        ("place_descend_closed", place_action, cfg.place_descend_steps),
        ("release_open", actions["place_open"], cfg.open_steps),
        ("retreat_open", actions["retreat_open"], cfg.retreat_steps),
        ("hold_open", actions["retreat_open"], cfg.hold_steps),
    ]
...
def summarize_trace(args: argparse.Namespace, env: WidowXTabletopEnv, obs: dict, trace: TimingTrace, attempts: int, attempt_summary: dict) -> dict:
    metrics = env.metrics()
    return {
        "version": VERSION,
        ...
        "ever_standard_grasp_success": bool(trace.ever_standard_grasp_success),
        "tcp_grasp_lift_success": bool(trace.ever_tcp_lift_success and trace.max_object_z >= float(args.lift_threshold)),
        "strict_grasp_lift_success": bool(trace.ever_standard_grasp_success and trace.max_object_z >= float(args.lift_threshold)),
        ...
        "min_ee_object_distance": trace.min_ee_object_distance,
        "min_ee_object_distance_while_lifted": trace.min_ee_object_distance_while_lifted,
        ...
        "first_standard_grasp_step": trace.first_standard_grasp_step,
        ...
        "stage_min_ee_distance": {key: float(value) for key, value in trace.stage_min_ee_distance.items()},
...
def rollout_with_env(args: argparse.Namespace, env: WidowXTabletopEnv, obs: dict, viewer=None) -> dict:
    if env.task.kind != "place":
        raise ValueError("gripper timing probe currently targets place tasks")
    ...
    for attempt in range(1, max(0, int(args.retries)) + 2):
        attempts = attempt
        attempt_summary = execute_attempt(args, env, expert, trace, viewer)
...
def main() -> None:
    ...
    strict = sum(1 for item in summaries if item["strict_grasp_lift_success"])
    ...
    print(f"strict_grasp_lift_rate: {strict}/{len(summaries)} = {strict / max(1, len(summaries)):.3f}", flush=True)
