...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a trained NumPy MLP BC policy in closed loop.")
...
def latest_model() -> Path:
    candidates = sorted((ROOT / "outputs" / "mlp_bc").glob("mlp_bc_*.npz"), key=lambda path: path.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(f"no MLP BC models found under {ROOT / 'outputs' / 'mlp_bc'}")
    return candidates[-1]
...
def forward(layers: list[dict[str, np.ndarray]], x: np.ndarray) -> np.ndarray:
    value = x
...
def predict_action(model: dict, observation: np.ndarray) -> np.ndarray:
    x = ((observation.astype(np.float32) - model["x_mean"]) / model["x_std"])[None, :]
