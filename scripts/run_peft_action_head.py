...
from train_peft_action_head import frozen_forward, residual_forward  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run an adapter/LoRA-style PEFT action-head policy.")
...
def latest_model() -> Path:
    candidates = sorted((ROOT / "outputs" / "peft_action_head").glob("*_action_head_lite_*.npz"), key=lambda path: path.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(f"no PEFT action-head models found under {ROOT / 'outputs' / 'peft_action_head'}")
    return candidates[-1]
...
def load_model(path: Path) -> dict:
    with np.load(path) as data:
        ...
        layers = []
        index = 0
        while f"base_w{index}" in data.files:
            layers.append({"w": data[f"base_w{index}"].astype(np.float32), "b": data[f"base_b{index}"].astype(np.float32)})
            index += 1
        params = {}
        for name in ("adapter_down", "adapter_down_b", "adapter_up", "adapter_up_b", "lora_a", "lora_b"):
            if name in data.files:
                params[name] = data[name].astype(np.float32)
        return {
            "layers": layers,
            "params": params,
...
def predict_action(model: dict, env: WidowXTabletopEnv, target_position: np.ndarray, phase: float, obs: dict) -> np.ndarray:
    observation = observation_from_env(env, target_position, phase=phase)[None, :]
    ...
    base_prediction, hidden = frozen_forward(model["layers"], x)
    residual, _ = residual_forward(model["metadata"]["mode"], model["params"], hidden, float(model["metadata"]["scale"]))
    y = (base_prediction + residual)[0].astype(np.float32)
...
def rollout_with_env(args: argparse.Namespace, model: dict, env: WidowXTabletopEnv, obs: dict, seed: int, task: str, complexity: str, viewer=None) -> dict:
    ...
    for step in range(args.steps):
        raw_action = predict_action(model, env, target_position, step / phase_denom, obs)
...
def main() -> None:
    ...
    print(f"mode: {model['metadata']['mode']}", flush=True)
    print(f"base_model: {model['metadata']['base_model']}", flush=True)
    print(f"trainable_params: {model['metadata']['trainable_params']}", flush=True)
    print(f"frozen_params: {model['metadata']['frozen_params']}", flush=True)
