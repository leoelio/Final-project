...
from train_torch_act import StateACTPolicy, append_phase_one_hot  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a PyTorch Transformer ACT-style action-chunk policy in closed loop.")
...
def latest_model() -> Path:
    candidates = sorted((ROOT / "outputs" / "torch_act").glob("torch_act_state_chunk_*.pt"), key=lambda path: path.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(f"no torch ACT models found under {ROOT / 'outputs' / 'torch_act'}")
    return candidates[-1]
...
def prepare_model_input(model: dict, observation_history: list[np.ndarray]) -> np.ndarray:
    ...
    raw_observation_dim = int(model["metadata"].get("raw_observation_dim", raw_history.shape[-1]))
    if model["metadata"].get("augment_relative", False):
        layout = model["metadata"].get("layout") or observation_layout()
        raw_history = augment_relative_features(raw_history, layout)
    if model["metadata"].get("phase_one_hot", False):
        raw_history = append_phase_one_hot(raw_history, phase_index=raw_observation_dim - 3)
...
def rollout_with_env(args: argparse.Namespace, model: dict, env: WidowXTabletopEnv, obs: dict, seed: int, task: str, complexity: str, viewer=None) -> dict:
    ...
    for step in range(args.steps):
        ...
        if float(metrics["object_z"]) >= lift_threshold:
            ...
            ever_tcp_lift_success = ever_tcp_lift_success or tcp_object_distance <= tcp_lift_threshold
