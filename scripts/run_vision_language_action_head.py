...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a frozen-visual-feature + language-token action-head policy.")
...
def latest_model() -> Path:
    root = ROOT / "outputs" / "vision_language_action_head"
    candidates = sorted(root.glob("vision_language_action_head_lite_*.npz"), key=lambda path: path.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(f"no vision-language action-head models found under {root}")
    return candidates[-1]
...
def predict_action(model: dict, env: WidowXTabletopEnv, renderer: mujoco.Renderer, obs: dict, phase: float) -> np.ndarray:
    features = feature_from_env_state(env, renderer, metadata_from_env(env, obs), phase, int(model["metadata"]["grid_size"]))
    x = ((features.astype(np.float32) - model["x_mean"]) / model["x_std"])[None, :]
...
def rollout_with_env(
    ...
    try:
        for step in range(args.steps):
            raw_action = predict_action(model, env, renderer, obs, step / phase_denom)
