...
from train_visual_act_cnn_cvae import VisualACTCNNCVAEPolicy, aux_from_env_state  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a lightweight CNN visual ACT-CVAE policy in closed loop.")
...
def latest_model() -> Path:
    root = ROOT / "outputs" / "visual_act_cnn_cvae"
    candidates = sorted(root.glob("visual_act_cnn_cvae_*.pt"), key=lambda path: path.stat().st_mtime)
    if not candidates:
        raise FileNotFoundError(f"no visual ACT-CVAE models found under {root}")
    return candidates[-1]
...
def load_model(path: Path) -> dict:
    ...
    return {
        "policy": policy,
        "aux_mean": checkpoint["aux_mean"].float().numpy(),
        "aux_std": checkpoint["aux_std"].float().numpy(),
...
def current_observation(model: dict, env: WidowXTabletopEnv, renderer: mujoco.Renderer, obs: dict, phase: float) -> tuple[np.ndarray, np.ndarray]:
    metadata = metadata_from_obs(env, obs)
    return render_rgb_chw(renderer, env), aux_from_env_state(env, metadata, phase)


def prepare_model_input(model: dict, image_history: list[np.ndarray], aux_history: list[np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    history = model_history(model)
    images = np.asarray(image_history[-history:], dtype=np.uint8)
    aux = np.asarray(aux_history[-history:], dtype=np.float32)
    if len(images) < history:
        image_pad = np.repeat(images[:1], history - len(images), axis=0)
        aux_pad = np.repeat(aux[:1], history - len(aux), axis=0)
        images = np.concatenate([image_pad, images], axis=0)
        aux = np.concatenate([aux_pad, aux], axis=0)
    aux_norm = ((aux - model["aux_mean"][None, :]) / model["aux_std"][None, :]).astype(np.float32)
    return images.astype(np.uint8), aux_norm


@torch.no_grad()
def predict_chunk(model: dict, images: np.ndarray, aux: np.ndarray) -> np.ndarray:
    image_tensor = torch.from_numpy(images[None, :, :, :, :])
    aux_tensor = torch.from_numpy(aux[None, :, :].astype(np.float32))
    y_norm = model["policy"].decode(image_tensor, aux_tensor)[0].cpu().numpy().astype(np.float32)
...
def rollout_with_env(args: argparse.Namespace, model: dict, env: WidowXTabletopEnv, obs: dict, seed: int, task: str, complexity: str, viewer=None) -> dict:
    ...
    try:
        for step in range(args.steps):
            image, aux = current_observation(model, env, renderer, obs, step / phase_denom)
            image_history.append(image)
            aux_history.append(aux)
            image_history = image_history[-model_history(model):]
            aux_history = aux_history[-model_history(model):]
            if step % replan_interval == 0:
                model_images, model_aux = prepare_model_input(model, image_history, aux_history)
                chunks.append((step, predict_chunk(model, model_images, model_aux)))
...
def main() -> None:
    ...
    print(f"image_size: {model['metadata']['image_size']}", flush=True)
